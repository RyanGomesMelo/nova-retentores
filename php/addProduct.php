...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $checkStmt = $conn->prepare("SELECT prod_id FROM product WHERE prod_id = ?");
    $checkStmt->bind_param("i", $prod_id);
    $checkStmt->execute();
    $result = $checkStmt->get_result();

    if ($result->num_rows > 0) {
        $message = 'ID do produto já existe. Escolha um ID diferente.';
    } else {
        if (isset($_FILES['prod_image']) && $_FILES['prod_image']['error'] === 0) {
            ...
            if (move_uploaded_file($_FILES["prod_image"]["tmp_name"], $target_file)) {
                $image_path = basename($_FILES["prod_image"]["name"]); 
            } else {
        ...
        $stmt = $conn->prepare("INSERT INTO product (prod_id, prod_name, prod_price, prod_description, cat_id, image_path) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isdsis", $prod_id, $prod_name, $prod_price, $prod_description, $cat_id, $image_path);
        ...
        <div class="form-container">
            <form method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    <label for="prod_id">ID do Produto</label>
                ...
                <div class="form-group">
                    <label for="prod_image">Imagem do Produto</label>
